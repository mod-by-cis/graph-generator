...
import { dotContentSignal, updateDotContent } from "../core/state-dot-current.ts";
...
const CodeExample = ({ title, code }: { title: string; code: string }) => {
  ...
  const handleShowExample = () => {
    ...
    if (stateSignal) {
      ...
      const renderPanelTitle = currentState.fieldTitles[4]; 
      ...
      if (renderPanelTitle) {
        ...
        stateSignal.value = {
          ...currentState,
          mode: 'single',
          visiblePanels: [renderPanelTitle, null],
        };
      } else {
        console.warn("Nie można znaleźć panelu 'Efekt..'. Sprawdź `viewID` w main.tsx.");
      }
    }
  };
  ...
  return (
    <div class="code-example">
      <div class="code-header">
        <span>{title}</span>
        <button onClick={handleShowExample} title="Wyświetl ten przykład w rendererze">Pokaż efekt</button>
      </div>
 ...
 */
const DocSection = ({ title, children }: { title: string, children: VNode | string }) => (
...
export function PageEduDot(): VNode {
  return (
    <div class="edu-page-wrapper">
      ...
      <p class="edu-intro">
        DOT to prosty język tekstowy służący do opisu grafów. Jego czytelna składnia pozwala na łatwe definiowanie węzłów, krawędzi i ich atrybutów, a silnik Graphviz automatycznie generuje z niego wizualizacje.
      </p>

      <DocSection title="1. Rodzaje Grafów">
        <p>
          Grafy mogą być skierowane (gdzie krawędzie mają kierunek, jak strzałki) lub nieskierowane (gdzie krawędzie to zwykłe połączenia).
        </p>
        <CodeExample
      ...
      <DocSection title="2. Definiowanie Węzłów (Nodes)">
        <p>
          Węzły to podstawowe elementy grafu. Można im nadawać etykiety i zmieniać ich wygląd za pomocą atrybutów w nawiasach kwadratowych `[...]`.
        </p>
        <CodeExample
      ...
      <DocSection title="3. Definiowanie Krawędzi (Edges)">
        <p>
          Krawędzie łączą węzły. Im również można nadawać atrybuty, takie jak etykiety, kolory czy style linii.
        </p>
        <CodeExample
      ...
      <DocSection title="4. Atrybuty Globalne i Subgrafy">
        <p>
          Można ustawić domyślne atrybuty dla wszystkich węzłów lub krawędzi w grafie. Subgrafy pozwalają grupować węzły.
        </p>
        <CodeExample
      ...
      <DocSection title="Przydatne Linki i Dalsza Nauka">
        <ul class="edu-links-list">
          <li>🔗 <a href="https://graphviz.org/documentation/" target="_blank" rel="noopener noreferrer">Oficjalna Dokumentacja Graphviz</a> - Najlepsze i najbardziej kompletne źródło wiedzy.</li>
          <li>🔗 <a href="https://graphviz.org/doc/info/attrs.html" target="_blank" rel="noopener noreferrer">Pełna Lista Atrybutów</a> - Wszystkie możliwe opcje do stylizacji węzłów, krawędzi i grafów.</li>
          <li>🔗 <a href="https://graphviz.org/gallery/" target="_blank" rel="noopener noreferrer">Galeria Przykładów</a> - Ogromna galeria inspirujących grafów stworzonych za pomocą DOT.</li>
        </ul>
      </DocSection>
    </div>
  );
